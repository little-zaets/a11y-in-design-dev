...
5. Apply **What a screenshot does not reveal** (below). For any element whose function falls into those gaps and is not made unambiguous by the user prompt or whole-page context, add it to an **unknowns** list. Do not infer activation behavior from appearance alone.
6. Use **Resolve ambiguity** to identify *which* ambiguity applies when function is unknown. The decision rules name the *question* to ask — they do not supply the *answer* when behavior is unstated.
7. **Mandatory clarification** — After steps 1–6, list every element whose function you **inferred** rather than **confirmed**. If your reasoning uses hedge words ("likely," "probably," "might," "could," "or" between two patterns, "need to determine"), that element is **not** confirmed. For each item, state your assumption and a targeted question. **Present this list to the user and obtain their answers or corrections. Do not begin Phase 2 until every item is resolved.** **Output contract:** When the unknowns list is non-empty, your **next user-facing message** must contain only the numbered questions (optionally with a brief restatement of what is unknown for each). Do not output HTML, CSS, JavaScript, or other implementation in that message. Do not state that you will "proceed with" defaults, typical patterns, or best guesses.

**Forbidden shortcuts** — None of the following replace confirmed function or satisfy **Mandatory clarification**:

- Industry or "common" UI patterns (e.g. "a hamburger usually opens a drawer," "a cart icon usually goes to the cart page")
- Reasonable defaults, best guesses, or "practical" choices to avoid blocking the task or to deliver implementation sooner
...
**Hard rule:** If Phase 1 produces a non-empty unknowns list, you must not start Phase 2 until the user has answered and every item is resolved.
...
When structured metadata and apparent function **agree**, classification is confirmed. When they **conflict** (e.g., a layer named "Carousel" that renders a static grid with no rotation controls), add the element to the **unknowns** list for mandatory clarification — do not silently choose one signal over the other. When structured metadata is **absent**, classify from apparent function and flag gaps per the rules below.

If the element's function is not evident from the available signals — for example, the user describes a control but does not specify what happens on activation — classification cannot reach 100% confidence. Proceed to **Resolve ambiguity** and the **Mandatory clarification** step in Phase 1.
...
These rules help you **identify** which ambiguity applies when classification is uncertain. They do **not** resolve unknown **function** by themselves: when activation behavior is unstated, each rule describes the **question** to ask the user, not the answer to assume.

If the Categories list and Interaction model table do not produce a confident classification, use these decision rules:
...
**Decorative vs meaningful image** — A graphic is **decorative** if removing it would not change the page's information or function; it is **meaningful** if it conveys information not available from surrounding text. For **`<img>`**, use `alt=""` when decorative and a non-empty `alt` when meaningful. For **image content not using `<img>`** (e.g. inline `<svg>`, `role="img"`), use `aria-label` or `aria-labelledby` when meaningful and `aria-hidden="true"` when decorative — see **Document structure — Images**.
...
Generate all markup according to the rules below. For every element, first follow the **Universal rules**, then follow the rules in the subsection matching the element's classification.
